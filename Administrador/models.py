...
class Categoria(models.Model):
    ...
    interna = models.BooleanField(default=False, verbose_name='interna', help_text=u'Señala si la categoria es de uso interno por lo que los scripts de automatizacion no la deben borrar')
    history = HistoricalRecords()
...
class TipoMime(models.Model):
    ...
    actualizaciones = models.BooleanField(default=False, verbose_name='actualizaciones', help_text=u'Señala si el tipo de MIME se debe permitir durante el modo actualización')
    history = HistoricalRecords()
...
class ExtensionArchivo(models.Model):
    ...
    actualizaciones = models.BooleanField(default=False, verbose_name='actualizaciones', help_text=u'Señala si la extension del archivo se debe permitir durante el modo de actualización')
    history = HistoricalRecords()
...
class Nivel(models.Model):
    nombre = models.CharField(max_length=256)
    filtrar = models.BooleanField(default=True, verbose_name='filtrar', help_text=u'Señala si el nivel debe ser procesado por el filtro de sitios web.')
    sinaccesoainternet = models.BooleanField(default=False, verbose_name='sin acceso a internet', help_text=u'Señala si el nivel no tiene permitido acceder a ningun sitio de internet')
    actualizaciones = models.BooleanField(default=True, verbose_name='actualizaciones', help_text=u'Señala si el nivel puede entrar en modo de actualización')
    lista_blanca = models.BooleanField(default=False, verbose_name='lista blanca', help_text=u'Señala si el nivel debe evaluar los filtros como una lista blanca')
    categorias = models.ManyToManyField(Categoria, blank=True, verbose_name='categorias')
    tipos_mime = models.ManyToManyField(TipoMime, blank=True, verbose_name='tipos de MIME')
    extensiones_archivos = models.ManyToManyField(ExtensionArchivo, blank=True, verbose_name='extensiones de archivos')
    history = HistoricalRecords()
...
class Asignacion(models.Model):
    IP_CHOICES = [(i, '192.168.2.%s' % i) for i in range(1, 255)]
    usuario = models.ForeignKey(Usuario)
    nivel = models.ForeignKey(Nivel)
    nombre_equipo = models.CharField(max_length=256, null=True, verbose_name='nombre del equipo', help_text=u'Nombre del equipo o Hostname. Ejemplo: PC1, MAC2, SRV3, SCAN4, etc.')
    equipo = models.ForeignKey(Equipo)
    tipo_conexion = models.PositiveSmallIntegerField(choices=TipoConexion, default=TipoConexion.LAN, validators=[MaxValueValidator(2)], verbose_name='tipo de conexion')
    ip = models.PositiveSmallIntegerField(choices=IP_CHOICES, unique=True, validators=[MaxValueValidator(254)], verbose_name='IP')
